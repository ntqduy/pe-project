...
path the full run will take, so nothing is "pilot-only" or "full-only".
    ...
    build_dataset(profile, paths)
        -> <derived>/datasets/<profile>/
             ...
             exclusions.csv        every study removed, with the rule that removed it
             integrity.json        corrupted / missing CT findings
             split_audit.json      official-split preservation + leakage guards
...
from .volumes import PreprocessingSpec, preprocess_study
...
def build_dataset(
    ...
    *,
    ...
    if preprocess:
        ...
        for record in cohort:
            try:
                ...
                failures.append({"study_id": record.study_id, "error": f"{type(exc).__name__}: {exc}"})
                continue
    ...
    _write_rows(
        destination / "exclusions.csv",
        ledger.entries,
        ["patient_id", "study_id", "impression_id", "split", "rule", "detail"],
    )
    _write_json(destination / "integrity.json", integrity_summary(checks))
    _write_json(destination / "split_audit.json", audit.as_dict())

    payload: dict[str, Any] = {
        "profile": dict(profile.get("profile") or {}),
        "source": source.provenance(),
        "scope": {"max_cases": max_cases, "allow_full": bool(allow_full)},
        "eligibility": eligibility_report.as_dict(),
        ...
        "integrity": {"level": str(integrity.get("level") or "path"), "limit": integrity_limit,
                      **integrity_summary(checks)},
        "adjudication": {"columns": list(adjudication_columns), "patients": len(labels)},
        "sampling": sampling_plan.as_dict() if sampling_plan else {"enabled": False},
